...
This small project implements an optimal 3D vertex merging algorithm (O(n+f) where n = number of vertices and f = number of triangles).
...
Example 1: a simple 2D mesh with repeated vertex locations, so that the simplified mesh can be validated visually.(run './build/polyclean mesh_ex1.dat')

Example 2: (in progress) a large randomly generated 3D mesh containing vertex clusters with a predefined diameter.
